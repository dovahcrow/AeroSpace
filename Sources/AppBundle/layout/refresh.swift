    ...
    debouncer.debounce(delay: Double(config.newWindowDetectionDebounce) / 1000, action: {
        ...
        td.run(
            task: {td in
                ...
                detectNewAppsAndWindows(startup: false)
            },
            onFinish: {td in
...
func refreshSession<T>(_ event: RefreshSessionEvent, screenIsDefinitelyUnlocked: Bool, startup: Bool = false, body: @escaping () -> T) -> T {
    ...
    if startup {
        ...
        if config.newWindowDetectionTimeout > 0 {
            maybeDetectNewWindows(event, screenIsDefinitelyUnlocked: screenIsDefinitelyUnlocked, startup: startup, body: body)
        } else {
            detectNewAppsAndWindows(startup: startup)
        }
    }
